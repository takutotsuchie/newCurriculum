...
func testTaskToManyTaskLabelRelations(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := TaskSlice{&a}
	if err = a.L.LoadTaskLabelRelations(ctx, tx, false, (*[]*Task)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testTaskToManyAddOpTaskLabelRelations(t *testing.T) {
	var err error
...
var (
	taskDBTypes = map[string]string{`ID`: `uuid`, `Title`: `text`, `Explanation`: `text`, `Limit`: `timestamp with time zone`, `Priority`: `integer`, `Status`: `text`, `UserID`: `uuid`}
	_           = bytes.MinRead
)
